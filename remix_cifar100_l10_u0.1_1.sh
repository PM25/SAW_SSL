...
# Variables
FOLDERNAME="l10_u0.1_s1"
DATASET="cifar100"       # "cifar10" for CIFAR-10, "cifar100" for CIFAR100
BASEDIR="experiments"
...
NUM_MAX=150   # 1500 for CIFAR-10, 150 for CIFAR-100
RATIO=2.0
IMB_L=10
IMB_U=-10
VAL_ITER=500
...
W_ROT=0.5
W_ENT=0.5
ALIGN=""    # "--align" to align distribution, "" to NOT align distribution
...
EST_PATH="./dist_estimation/cifar100_n150_m300_l10_u0.1_long_s1/MLLS_BCTS_estimation.json"
ITER_T=10
NUM_ITER=10
SEED=1
...
mkdir ${BASEDIR}/${DATASET}/RemixMatch
mkdir $FOLDERDIR 
...
\n \
# Hyperparameters for RemixMatch \n \
Mixup Beta Distribution Hyperparameter (alpha)  = $MIX_ALPHA \n \
EMA Decay Hyperparameter                        = $EMA_DECAY \n \
Sharpening Temperature Parameter (T)            = $SHARP_T \n \
Weight for Unsupervised Loss (Lambda_u)         = $LAMBDA_U \n \
Weight for Rotation Loss (W_ROT)                = $W_ROT \n \
Weight for Strong Augmentation Loss (W_ENT)     = $W_ENT \n \
\n \
# Hyperparameters for DARP \n \
Warmup Epoch                              = $WARM \n \
...
# Execute Experiment
python3 train_remix_${DATASET}.py \
--epochs $EPOCHS \
--start-epoch $START \
--batch-size $BATCH \
--lr $LEARNING_RATE \
