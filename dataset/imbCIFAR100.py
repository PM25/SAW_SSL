...
# Parameters for data
cifar100_mean = (0.5071, 0.4866, 0.4409) # equals np.mean(train_set.train_data, axis=(0,1,2))/255
...
def get_cifar100(root, l_samples, u_samples, test_samples, transform_train=transform_train, transform_strong=transform_strong,
    ...
    datasets["vanila"] = torchvision.datasets.CIFAR100(root, train=True, transform=transform_Tensor)

    # Generate Imbalanced Test Dataset
    base_dataset = torchvision.datasets.CIFAR100(root, train=False, download=download)
    ...
    # Training Datasets
    datasets["labeled"] = CIFAR100_labeled(root, train_labeled_idxs, train=True, transform=transform_train)
    datasets["unlabeled"] = CIFAR100_unlabeled(root, train_unlabeled_idxs, train=True,
                                                transform=TransformTwice(transform_train, transform_strong))

    # Test Datasets
    datasets["Test"] = CIFAR100_labeled(root, train=False, transform=transform_val, download=False)
    datasets["Imbalanced"] = CIFAR100_labeled(root, imb_test_idxs, train=False, transform=transform_val, download=False)
    datasets["Reversed"] = CIFAR100_labeled(root, reverse_test_idxs, train=False, transform=transform_val, download=False)

    # Fix Match Test Sets
    datasets["Weak"] = CIFAR100_labeled(root, train=False, transform=transform_train, download=False)
    datasets["Strong"] = CIFAR100_labeled(root, train=False, transform=transform_strong, download=False)
...
if __name__ == '__main__':
    ...
    datasets = get_cifar100('./data', N_SAMPLES_PER_CLASS,
