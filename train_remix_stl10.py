...
# import dataset.imbCIFAR10 as dataset
import dataset.imbSTL10_dojo as dataset
from utils import Bar, Logger, AverageMeter, accuracy, mkdir_p, savefig
# from tensorboardX import SummaryWriter
from scipy import optimize

from imbOptim.classWeights import parseClassWeights, createSettings, getClassWeights
from dataset.dataTools import gtDict, prob2Distribution, make_imb_data, prepareDataLoaders
from sslAlgo.mixLogger import createLogger, loadLogger, appendLogger, closeLogger
...
parseClassWeights(parser)   # Class Weights Arguments
...
def main():
    ...
    datasets = dataset.get_stl10('./data', N_SAMPLES_PER_CLASS, IMB_TEST_PER_CLASS, method='remix')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_loss, train_loss_x, train_loss_u,\
             emp_distb_u, pseudo_orig, pseudo_refine, \
                 distb_dict["pseudo"], distb_dict["darp"] = train(dataLoaders["labeled"],
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion, epoch, use_cuda,
    ...
    for batch_idx in range(args.val_iteration):
        ...
        targets_x = torch.zeros(batch_size, num_class).scatter_(1, targets_x.view(-1,1).type(torch.int64), 1)
        if use_cuda:
        ...
        Lx, Lu, w = criterion(logits_x, mixed_target[:batch_size], logits_u, mixed_target[batch_size:],\
             epoch+batch_idx/args.val_iteration, lambda_u, class_weight_x, class_weight_u)
...
class SemiLoss(object):
    def __call__(self, outputs_x, targets_x, outputs_u, targets_u, epoch, lambda_u, weights_x, weights_u):
        ...
        SCE_u = torch.sum(WCE_u, dim=1)    # Summed Cross-Entropy based on Quality, torch.Size([128])
        Lu = -torch.mean(SCE_u)     # Final Unsupervised Cross-Entropy Loss
