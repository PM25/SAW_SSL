...
# Method options
parser.add_argument('--num_max', type=int, default=450,
...
num_class = 10 # STL-10
test_num_max = 8000/num_class  # STL-10 per class
...
def main():
    ...
    IMB_TEST_PER_CLASS = make_imb_data(test_num_max, num_class, args.imb_ratio_l) # test dataset
    ...
    # datasets = dataset.get_stl10('./data', N_SAMPLES_PER_CLASS,
                                                        # U_SAMPLES_PER_CLASS, IMB_TEST_PER_CLASS)
    datasets = dataset.get_stl10('./data', N_SAMPLES_PER_CLASS, IMB_TEST_PER_CLASS,
                                                        method='fix')
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion, epoch, use_cuda,
    ...
    for batch_idx in range(args.val_iteration):
        ...
        with torch.no_grad():
            ...
            weight_u = class_weight_u.repeat(targets_u.size()[0], 1)
        ...
        # A: Fix Match
        targets_u = torch.mul(targets_u, weight_u)
        targets_u = F.normalize(targets_u, p=1, dim=1)
